<!DOCTYPE html>
<html lang="en">
<head>
  <title>Studenti</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.0/css/bootstrap.min.css">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.0/umd/popper.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.0/js/bootstrap.min.js"></script>
    <style>
   #header-fixed {
    position: fixed;
    top: 0px; display:none;
    background-color:white;
}
        p.user
        {
            padding-right:50px;
            text-align: right;
        }
    </style>
     <script>
         function anuntz() {
             if(document.getElementById("anuntz").innerHTML == "[MODIFICA]")
                 
                     
                 
             {
                 
                 document.getElementById("anuntz").innerHTML = "[SALVEAZA]";
                 document.getElementById("anunt").contentEditable = "true";
             }
             else 
             {
                   window.location.href="http://jock123.esy.es/proiect_final/anunt.php?anunt='"+document.getElementById("anunt").textContent+"'";
                
                 document.getElementById("anuntz").innerHTML = "[MODIFICA]";
                 document.getElementById("anunt").contentEditable = "false";
             }
                 
           
}
      function delete1(id) {
                   var r = confirm("Sigur vrei sa stergi intrarea ?");
          if (r == true) {
        window.location.href="http://jock123.esy.es/proiect_final/delete.php?id="+id;
    } else {
        window.location.href="http://jock123.esy.es/proiect_final/";
    }
      
    }
          function modify(id)
        {
           // header('Location: http://alexsejoaca.esy.es/projject/update_data.php?id='+id);
           
            if(document.getElementById("tdn"+id).contentEditable=='true')
            document.getElementById("tdn"+id).contentEditable='false';
            else document.getElementById("tdn"+id).contentEditable='true';
            
             if(document.getElementById("tdm"+id).contentEditable=='true')
            document.getElementById("tdm"+id).contentEditable='false';
            else document.getElementById("tdm"+id).contentEditable='true';
            
            
             if(document.getElementById("tdr"+id).contentEditable=='true')
            document.getElementById("tdr"+id).contentEditable='false';
            else document.getElementById("tdr"+id).contentEditable='true';
            
                    if(document.getElementById("tdg"+id).contentEditable=='true')
            document.getElementById("tdg"+id).contentEditable='false';
            else document.getElementById("tdg"+id).contentEditable='true';
            
               if(document.getElementById("tdis"+id).contentEditable=='true')
            document.getElementById("tdis"+id).contentEditable='false';
            else document.getElementById("tdis"+id).contentEditable='true';
            
                if(document.getElementById("tde"+id).contentEditable=='true')
            document.getElementById("tde"+id).contentEditable='false';
            else document.getElementById("tde"+id).contentEditable='true';
            
            if(document.getElementById("tdf"+id).contentEditable=='true')
            document.getElementById("tdf"+id).contentEditable='false';
            else document.getElementById("tdf"+id).contentEditable='true';
            
            if(document.getElementById("tdin"+id).contentEditable=='true')
            {
                
   
                document.getElementById("tdin"+id).contentEditable='false';
                
                var nume = document.getElementById("tdn"+id).textContent;
                var mate = document.getElementById("tdm"+id).textContent;
                var romana = document.getElementById("tdr"+id).textContent;
                var geografie = document.getElementById("tdg"+id).textContent;
                var istorie = document.getElementById("tdis"+id).textContent;
                var engleza = document.getElementById("tde"+id).textContent;
                var franceza = document.getElementById("tdf"+id).textContent;
                var informatica = document.getElementById("tdin"+id).textContent;
           
               
                
                window.location.href = "http://jock123.esy.es/proiect_final/update.php?id="+id+"&nume="+nume+"&mate="+mate+"&romana="+romana
                +"&geografie="+geografie+"&istorie="+istorie+"&engleza="+engleza+"&franceza="+franceza+"&informatica="+informatica;

            }
            else 
                document.getElementById("tdin"+id).contentEditable='true';
            // if(document.getElementById("tdin"+id).contentEditable=='false')
                
                
        }
         function fct()
         {
             
             document.getElementById("cuser").innerHTML = "Conectat ca vizitator";
             var kap = <?php echo setcookie("user_cookie", $row["nume"], time()-3600, "/proiect_final/");?>;
             window.location.href = "http://jock123.esy.es/proiect_final/user.php";
         }
    </script>
</head>
<body>
  <?php
  //  session_start([
  //  'cookie_lifetime' => 86400,
//]);
// if($_SESSION['name']==null)
       // {
// header('Location: http://jock123.esy.es/proiect_final/error.php');
  
  //  }
    ?>
        
        <?php
    
$servername = "mysql.hostinger.ro";
$username = "u872290312_inter";
$password = "alexutzu";
$dbname="u872290312_inter";
    
    $conn = mysqli_connect($servername, $username, $password, $dbname);


// Check connection
if (!$conn) {
    die("Connection failed: " . mysqli_connect_error());
}
                                    

    
    // output data of each row
?>
    
        <p class="user" onclick="fct()" id="cuser"><?php
            echo "Conectat ca ";
        if( $_COOKIE["user_cookie"] == true)
            echo $_COOKIE["user_cookie"]." (deconectare)";
            else echo "vizitator";
            ?></p>
<div class="container">
  <h2>Anunturi</h2>
  <p  id="anunt" contenteditable="false">
      
      
      
      
      
      <?php
      
    $sql="SELECT * from inter_proiect_student_anunt";
        
$result = $conn->query($sql);
        $i=0;
if ($result->num_rows >= 0) {
    $i = 1;
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo nl2br($row['anunt']);
    }
}

      ?>
      </p>
  <b>  <div  id="anuntz" onclick="anuntz()">[MODIFICA]</div></b>
      <br/>
  <table class="table  table-hover header-fixed">
    <thead>
      <tr>
        <th class="col-xs-2" align="center">#</th>
        <th class="col-xs-2" align="center">Nume</th>
        <th class="col-xs-2" align="center">Matematica</th>
        <th class="col-xs-2" align="center">Romana</th>
       <th class="col-xs-2" align="center">Geografie</th>
        <th class="col-xs-2" align="center">Istorie</th>
       <th class="col-xs-2" align="center">Engleza</th>
       <th class="col-xs-2" align="center">Franceza</th>
       <th class="col-xs-2" align="center">Informatica</th>
          <th class="col-xs-2" align="center">Edit</th>
        
      </tr>
    </thead>
    <tbody>
        <?php
    $sql="SELECT * from inter_proiect_student";
        
$result = $conn->query($sql);
        $i=0;
if ($result->num_rows >= 0) {
    $i = 1;
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr>
        <td>".$i."</td>
        <td align=\"center\" contenteditable='false' id=\"tdn".$row["id"]."\">".$row['nume']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdm".$row["id"]."\">".$row['mate']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdr".$row["id"]."\">".$row['romana']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdg".$row["id"]."\">".$row['geografie']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdis".$row["id"]."\">".$row['istorie']."</td>
        <td align=\"center\" contenteditable='false' id=\"tde".$row["id"]."\">".$row['engleza']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdf".$row["id"]."\">".$row['franceza']."</td>
        <td align=\"center\" contenteditable='false' id=\"tdin".$row["id"]."\">".$row['informatica']."</td>
     
        
        <td align=\"center\">
         <a onClick=modify(".$row["id"].")>
        <img src=\"right.ico\" height=\"31\" width=\"31\"/>
        </a>
        <a onClick=\"delete1(".$row["id"].")\">
    <img src=\"wrong.png\" height=\"31\" width=\"31\"/>
    </a>
        </td>
   
      </tr>";
        $i++;
    }
       

}
                                    
                                    mysqli_close($conn);
        
       
    ?>
      <tr>
      <th scope="row"></th>
      <td colspan="2"></td>
      <td colspan="2"></td>
     
     
      <td colspan="2">
      
      
      
      
          <button type="button"  data-toggle="modal" data-target="#myModal">+</button>
          </td>
      <td colspan="2"> </td>
          
      <td colspan="2"></td>
      
     
      
    </tr>
    
    </tbody>
  </table>
</div>
<div class="container">
  
  <!-- Trigger the modal with a button -->
  

  <!-- Modal -->
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog">
    
      <!-- Modal content-->
      <div class="modal-content">
         <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Adaugare student</h4>
        </div>
        <div class="modal-body content">
          <form method="post" action="add.php">
              <label>
              Nume 
              </label><br>
              <input type="text" name="nume" required><br/>
                <label>
              Nota matematica 
              </label><br>
              <input type="text" name="mate" required onkeydown="keyCode(event)" id="input_mate" maxlength="1"><label  id="label_mate"></label><br/>
              <label>
              Nota romana 
              </label><br>
              <input type="text" name="rom" required onkeydown="keyCode1(event)" id="input_romana" maxlength="1"><label  id="label_romana"></label><br/>
              <label>
              Nota geografie 
              </label><br>
              <input type="text" name="geo" required onkeydown="keyCode2(event)" id="input_geografie" maxlength="1"><label  id="label_geografie"></label><br/>
              <label>
              Nota istorie 
              </label><br>
              <input type="text" name="isto" required onkeydown="keyCode3(event)" id="input_istorie" maxlength="1"><label  id="label_istorie"></label><br/>
              <label>
              Nota engleza 
              </label><br>
              <input type="text" name="engleza" onkeydown="keyCode4(event)" id="input_engleza" maxlength="1"><label  id="label_engelza"></label><br/>
              <label>
              Nota franceza 
              </label><br>
              <input type="text" name="franceza" required onkeydown="keyCode5(event)" id="input_franceza" maxlength="1"><label  id="label_franceza"></label><br/>
              <label>
              Nota informatica 
              </label><br>
              <input type="text" name="informatica" required onkeydown="keyCode6(event)" id="input_info" maxlength="1"><label  id="label_info"></label><br/>
              
            <br/>
            <input type="submit" name="submit" value="ADD">
            </form>
            
            
            
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        </div>
      </div>
      
    </div>
  </div>
    </div>
    
    
    <script>
        var verif = 0;
        var final = 0;
function keyCode(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_mate").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_mate").value;

    if(doc=='1')
        {
        $("#input_mate").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_mate").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_mate").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_mate").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode1(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_romana").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_romana").value;

    if(doc=='1')
        {
        $("#input_romana").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_romana").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_romana").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_romana").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode2(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_geografie").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_geografie").value;

    if(doc=='1')
        {
        $("#input_geografie").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_geografie").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_geografie").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_geografie").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode3(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_istorie").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_istorie").value;

    if(doc=='1')
        {
        $("#input_istorie").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_istorie").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_istorie").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_istorie").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode4(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_engleza").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_engleza").value;

    if(doc=='1')
        {
        $("#input_engleza").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_engleza").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_engleza").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_engleza").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode5(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_franceza").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_franceza").value;

    if(doc=='1')
        {
        $("#input_franceza").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_franceza").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_franceza").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_franceza").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
        function keyCode6(event) {
    var x = event.keyCode;
   
    if(!verif)
    document.getElementById("label_info").innerHTML =null;
    if(x>=48 && x<=57)
    var doc = document.getElementById("input_info").value;

    if(doc=='1')
        {
        $("#input_info").attr("maxlength", 2);
            
            verif = 1;
            document.getElementById("label_info").innerHTML =null;
        }
   else verif = 0;
    if(verif == 1)
        {
            if(x!=48)
            {document.getElementById("label_info").innerHTML ="  invalid"; verif  =0;}
            else document.getElementById("label_info").innerHTML =null;
            final = 1;
        }
     
    console.log("verif este : "+verif);
    console.log(x);
        
}
       
</script>
    
</body>
</html>
